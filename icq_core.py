...
_RTF_SKIP_DESTINATIONS = {
    ...
    "object", "footnote", "header", "footer", "headerl", "headerr",
    "footerl", "footerr", "filetbl", "listtable", "revtbl", "rsidtbl",
    "themedata", "colorschememapping", "latentstyles", "datastore",
    "xmlnstbl", "wgrffmtfilter", "panose", "shpinst", "sp", "nonshppict",
    "blipuid", "bkmkstart", "bkmkend", "listoverridetable", "listlevel",
}
_RTF_CODEPAGE_MAP = {
    1250: "cp1250", 1251: "cp1251", 1252: "cp1252", 1253: "cp1253",
    1254: "cp1254", 1255: "cp1255", 1256: "cp1256", 1257: "cp1257",
    1258: "cp1258", 65001: "utf-8",
}
def _rtf_to_text(raw: bytes) -> Optional[str]:
    ...
    codepage = int(m.group(1)) if m else 1251
    enc = _RTF_CODEPAGE_MAP.get(codepage, "cp1251")
    out = bytearray()
    depth = 0
    skip_from = None
    i, n = 0, len(s)
    while i < n:
        ch = s[i]
        if ch == "\\":
            m2 = re.match(r"\\'([0-9a-fA-F]{2})", s[i:i + 4])
            if m2:
                if skip_from is None:
                    out.append(int(m2.group(1), 16))
                i += len(m2.group(0))
                continue
            m2 = re.match(r"\\([a-zA-Z]+)(-?\d+)?( )?", s[i:i + 40])
            if m2:
                word = m2.group(1)
                if skip_from is None:
                    if word in ("par", "line"):
                        ...
                        out.extend(b"\t")
                    elif word == "u" and m2.group(2) is not None:
                        try:
                            cp = int(m2.group(2))
                            if cp < 0:
                                cp += 65536
            ...
            if i + 1 < n:
                if skip_from is None:
                    out.extend(s[i + 1].encode("latin-1", errors="ignore"))
                i += 2
            else:
                i += 1
            continue
        if ch == "{":
            depth += 1
            m3 = re.match(r"\{(\\\*)?\\([a-zA-Z]+)", s[i:i + 40])
            word = m3.group(2) if m3 else None
            if skip_from is None and word in _RTF_SKIP_DESTINATIONS:
                skip_from = depth
            i += 1
            continue
        if ch == "}":
        ...
        i += 1
    try:
        text = bytes(out).decode(enc, errors="replace")
    except LookupError:
        text = bytes(out).decode("cp1251", errors="replace")
    return text.strip()
def _decode_text(raw: bytes) -> str:
    clean = raw.rstrip(b"\x00").strip()
    if not clean:
        return ""
    if clean.lstrip().startswith(b"{\\rtf"):
        rtf_text = _rtf_to_text(clean)
...
CLI_META_REQINFO_TYPE    = 0x04D0   
CLI_META_REQMOREINFO_TYPE= 0x04B2   
CLI_META_REQWORKINFO_TYPE= 0x04BA   
CLI_META_REQABOUT_TYPE   = 0x04C8   
CLI_SET_FULLINFO_TYPE    = 0x0C3A   
SRV_META_GENERAL_TYPE    = 0x00C8   
SRV_META_MORE_TYPE       = 0x00DC   
SRV_META_WORK_TYPE       = 0x00D2   
SRV_META_ABOUT_TYPE      = 0x00E6   
SRV_META_END_TYPE        = 0x00FA   
...
META_INFO_PASS_ACK     = 0x00AA   
CLI_ROSTERADD_CMD    = 0x0008   
CLI_ROSTERDELETE_CMD = 0x000A   
SRV_UPDATEACK_CMD    = 0x000E   
CLI_ADDSTART_CMD     = 0x0011   
CLI_ADDEND_CMD       = 0x0012   
CLI_ROSTERUPDATE_CMD = 0x0009   
META_REQ_OFFLINE_MSG  = 0x003C  
META_ACK_OFFLINE_MSG  = 0x003E  
OFFLINE_MSG_RESPONSE  = 0x0041  
OFFLINE_MSG_EOF       = 0x0042  
SSI_AUTH_SEND_REQ   = 0x0018   
SSI_AUTH_REQ        = 0x0019   
SSI_AUTH_SEND_REPLY = 0x001A   
SSI_AUTH_REPLY      = 0x001B   
SSI_YOU_WERE_ADDED  = 0x001C   
...
def _make_meta_snac(my_uin: str, subtype: int,
    ...
    uin_int = int(my_uin)
    inner = bytearray()
    inner += struct.pack("<I", uin_int)   
    inner += struct.pack("<H", 0x07D0)   
    inner += struct.pack("<H", seq_id)   
    inner += struct.pack("<H", subtype)  
    inner += data                        
    this_data_len = 2 + len(data)  
...
def _decode_ssi_nick(raw: bytes) -> str:
    if not raw:
        return ""
    return _try_decode(raw).strip("\x00").strip()
